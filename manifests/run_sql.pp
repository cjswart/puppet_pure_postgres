...
define pure_postgres::run_sql (
  ...
  $run_cmd    = shellquote("${pure_postgres::params::pg_bin_dir}/psql", '-c', $sql)
  if $unless {
    $unless_sql = shellquote( $unless )
    $unless_cmd = "/bin/test $(${pure_postgres::params::pg_bin_dir}/psql --quiet --tuples-only -c ${unless_sql} | wc -l) -gt 1"
  }  else {
    $unless_cmd = undef
  }
